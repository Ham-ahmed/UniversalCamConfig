...
# === CORRECTION START ===
# PRIMARY DOWNLOAD: Try the main archive directly FIRST.
# If the project is no longer using numbered releases, this is the correct file.
DOWNLOAD_FILENAME="UniversalCamConfig_v${version}.tar.gz"
DOWNLOAD_URLS=(
    # Primary: Direct archive from the main branch
    "${GITHUB_BASE}/archive/main.tar.gz"
    # Primary Alternative: Direct archive using refs/heads
    "${GITHUB_BASE}/archive/refs/heads/main.tar.gz"
    # Fallback 1: The traditional numbered release URL (might not exist)
    "https://github.com/Ham-ahmed/UniversalCamConfig/releases/download/v${version}/UniversalCamConfig_v${version}.tar.gz"
    # Fallback 2: The 'latest' release URL
...
# If the main.tar.gz file was downloaded, rename it for consistent handling.
if [ $DOWNLOAD_SUCCESS -eq 1 ] && echo "$url" | grep -q "main.tar.gz"; then
    print_message $YELLOW "> Downloaded main.tar.gz, renaming to expected package file."
    # Keep the original if needed for extraction
    cp "$DOWNLOAD_FILENAME" "main_source.tar.gz"
    # Ensure DOWNLOAD_FILENAME has the correct name for the rest of the script
    mv "$DOWNLOAD_FILENAME" "UniversalCamConfig_v${version}.tar.gz"
fi

# Final fallback attempt if all URLs fail
...
if [ ! -f "UniversalCamConfig_v${version}.tar.gz" ]; then
    print_message $RED "> Downloaded file not found!"
    exit 1
fi

# Check file size
FILESIZE=$(stat -c%s "UniversalCamConfig_v${version}.tar.gz" 2>/dev/null || wc -c < "UniversalCamConfig_v${version}.tar.gz" 2>/dev/null || echo "0")
if [ "$FILESIZE" -lt 100 ]; then
    print_message $RED "> Downloaded file is too small (${FILESIZE} bytes), may be corrupted!"
    rm -f "UniversalCamConfig_v${version}.tar.gz"
    exit 1
fi
...
if ! tar -xzf "UniversalCamConfig_v${version}.tar.gz" 2>/dev/null; then
    print_message $YELLOW "> Standard extraction failed, trying alternative method..."
    
    # Try gzip then tar separately
    if gzip -d "UniversalCamConfig_v${version}.tar.gz" 2>/dev/null; then
        TAR_FILE="UniversalCamConfig_v${version}.tar"
        if [ -f "$TAR_FILE" ]; then
...
if [ -d "$PLUGINPATH" ] && [ -f "$PLUGINPATH/__init__.py" ]; then
    ...
    if [ -n "$FOUNDPATH" ] && [ -f "$FOUNDPATH/__init__.py" ]; then
        ...
        tar -xzf "UniversalCamConfig_v${version}.tar.gz" -C / 2>/dev/null
        if [ -d "$PLUGINPATH" ] && [ -f "$PLUGINPATH/__init__.py" ]; then
